...
@baseline_registry.register_trainer(name="ddppo-eme")
...
class EMETrainer(BaseRLTrainer):
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config) -> None:
        ...
        if self._is_distributed:
            ...
            self.inverse_dynamics = DDP(self.inverse_dynamics, device_ids=[self.device], output_device=self.device,\
                                        find_unused_parameters=False)
        self.encoder_optimizer = torch.optim.Adam(self.encoder.parameters(), lr=self.config.RL.eme.encoder_lr)
        self.inverse_dynamics_optimizer = torch.optim.Adam(self.inverse_dynamics.parameters(), lr=self.config.RL.eme.encoder_lr)
    ...
    def _init_train(self):
        ...
        ridge = self.config.RL.eme.ridge
    ...
    def _compute_actions_and_step_envs(self, buffer_index: int = 0):
        ...
        with torch.no_grad():
            if self.config.RL.eme.embedding == 'idm':
    ...
    def _collect_environment_result(self, buffer_index: int = 0):
        ...
        for i, done in enumerate(dones):
            if done:
                self.inverse_covs[i].copy_(torch.eye(self.inverse_covs.shape[1]) * (1./ self.config.RL.eme.ridge))
                

        # add intrinsic and extrinsic rewards together
        rewards = rewards + self.config.RL.eme.bonus_coef * bonus_rewards
    ...
    def _update_agent(self):
        ...
        # eme stuff
        inv_dynamics_loss = 0
        for i in range(self.config.RL.eme.inv_dynamics_epochs):
        ...
        if self.config.RL.eme.inv_dynamics_epochs > 0:
            inv_dynamics_loss /= self.config.RL.eme.inv_dynamics_epochs
